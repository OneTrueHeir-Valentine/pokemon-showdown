...
export const Moves: import('../sim/dex-moves').MoveDataTable = {
	...
	absorb: {
		...
		pp: 25,
		priority: 0,
		flags: {protect: 1, mirror: 1, heal: 1, metronome: 1, drain: 1},
		drain: [1, 1],
		secondary: null,
		target: "normal",
		type: "Grass",
	...
	alluringvoice: {
		...
		pp: 10,
		priority: 0,
		flags: {protect: 1, mirror: 1, vibration: 1, bypasssub: 1, metronome: 1},
		secondary: {
			chance: 100,
	...
	galliumdrip: {
		...
		priority: 0,
		flags: {snatch: 1, metronome: 1},
		volatileStatus: 'galliumdrip',
		condition: {
			onStart(pokemon) {
				this.add('-start', pokemon, 'Gallium Drip');
			},
			onResidualOrder: 6,
			onResidual(pokemon) {
				this.heal(pokemon.baseMaxhp / 8);
	...
	epoxyresin: {
		...
		priority: 0,
		flags: {snatch: 1, metronome: 1},
		volatileStatus: 'epoxyresin',
		condition: {
			onStart(pokemon) {
				this.add('-start', pokemon, 'Epoxy Resin');
			},
			onResidualOrder: 6,
			onResidual(pokemon) {
				this.heal(pokemon.baseMaxhp / 8);
	...
	ionsphere: {
		...
		priority: 0,
		flags: {snatch: 1, metronome: 1},
		volatileStatus: 'ionsphere',
		condition: {
			onStart(pokemon) {
				this.add('-start', pokemon, 'Ion Sphere');
			},
			onResidualOrder: 6,
			onResidual(pokemon) {
				this.heal(pokemon.baseMaxhp / 8);
	...
	draconicrestoration: {
		...
		priority: 0,
		flags: {snatch: 1, metronome: 1},
		volatileStatus: 'draconicrestoration',
		condition: {
			onStart(pokemon) {
				this.add('-start', pokemon, 'Draconic Restoration');
			},
			onResidualOrder: 6,
			onResidual(pokemon) {
				this.heal(pokemon.baseMaxhp / 8);
	...
	auroraveil: {
		...
		onTry() {
			return this.field.isWeather(['haildance', 'snowdance']);
		},
		condition: {
			duration: 6,
	...
	bitterblade: {
		...
		pp: 10,
		priority: 0,
		flags: {contact: 1, protect: 1, mirror: 1, heal: 1, metronome: 1, slicing: 1, drain: 1},
		drain: [1, 2],
		secondary: null,
		target: "normal",
		type: "Fire",
	...
	blizzard: {
		...
		flags: {protect: 1, mirror: 1, metronome: 1, wind: 1},
		onModifyMove(move) {
			if (this.field.isWeather(['haildance', 'snowdance'])) move.accuracy = true;
		},
		secondary: {
			chance: 10,
			status: 'frz',
	...
	bouncybubble: {
		...
		pp: 20,
		priority: 0,
		flags: {protect: 1, mirror: 1, heal: 1, drain: 1},
		drain: [1, 1],
		secondary: null,
		target: "normal",
		type: "Water",
	...
	chatter: {
		...
		priority: 0,
		flags: {
			protect: 1, mirror: 1, vibration: 1, distance: 1, bypasssub: 1,
	...
	clangoroussoul: {
		...
		pp: 5,
		priority: 0,
		flags: {snatch: 1, vibration: 1, dance: 1},
		onTry(source) {
	...
	confide: {
		...
		pp: 20,
		priority: 0,
		flags: {reflectable: 1, mirror: 1, vibration: 1, bypasssub: 1, metronome: 1},
		boosts: {
			spa: -1,
		},
		secondary: null,
	...
	hex: {
		...
		onTryHit(target, source, move) {
			if (!source.hasType('Ghost')) {
				delete move.volatileStatus;
				delete move.onHit;
				move.self = {boosts: {spe: -1, spa: 1, spd: 2}};
			} else if (move.volatileStatus && target.volatiles['curse']) {
				return false;
			}
		},
	...
	dig: {
		...
		condition: {
			...
			onImmunity(type, pokemon) {
				if (type === 'sandstorm' || type === 'haildance') return false;
			},
			onInvulnerability(target, source, move) {
	...
	drainingkiss: {
		...
		pp: 10,
		priority: 0,
		flags: {contact: 1, protect: 1, mirror: 1, heal: 1, metronome: 1, drain: 1},
		drain: [1, 1],
		secondary: null,
		target: "normal",
		type: "Fairy",
	...
	drainpunch: {
		...
		pp: 10,
		priority: 0,
		flags: {contact: 1, protect: 1, mirror: 1, punch: 1, heal: 1, metronome: 1, drain: 1},
		drain: [1, 2],
		secondary: null,
		target: "normal",
		type: "Fighting",
	...
	firespin: {
		...
		pp: 15,
		priority: 0,
		flags: {protect: 1, mirror: 1, metronome: 1, spin: 1},
		volatileStatus: 'partiallytrapped',
		secondary: null,
		target: "normal",
		type: "Fire",
	...
	gmaxsteelsurge: {
		...
		condition: {
			...
			onEntryHazard(pokemon) {
				if (pokemon.hasItem('heavydutyboots') || pokemon.hasItem('battlearmor')) return;
	...
	grasswhistle: {
		...
		pp: 15,
		priority: 0,
		flags: {protect: 1, reflectable: 1, mirror: 1, vibration: 1, bypasssub: 1, metronome: 1},
		status: 'slp',
		secondary: null,
		target: "normal",
		type: "Grass",
	...
	healbell: {
		...
		pp: 5,
		priority: 0,
		flags: {snatch: 1, vibration: 1, distance: 1, bypasssub: 1, metronome: 1},
		onHit(target, source) {
	...
	howl: {
		...
		pp: 40,
		priority: 0,
		flags: {snatch: 1, vibration: 1, metronome: 1},
		boosts: {
			atk: 1,
		},
		secondary: null,
	...
	hurricane: {
		...
		pp: 10,
		priority: 0,
		flags: {protect: 1, mirror: 1, distance: 1, metronome: 1, wind: 1, spin: 1},
		onModifyMove(move, pokemon, target) {
	...
	matchagotcha: {
		...
		pp: 10,
		priority: 0,
		flags: {protect: 1, mirror: 1, defrost: 1, heal: 1, metronome: 1, drain: 1},
		drain: [1, 2],
		thawsTarget: true,
		secondary: {
			chance: 20,
	...
	maxhaildancestorm: {
		...
		self: {
			onHit(source) {
				if (!source.volatiles['dynamax']) return;
				this.field.setWeather('haildance');
			},
		},
		target: "adjacentFoe",
		type: "Ice",
	...
	metalsound: {
		...
		pp: 40,
		priority: 0,
		flags: {protect: 1, reflectable: 1, mirror: 1, vibration: 1, bypasssub: 1, allyanim: 1, metronome: 1},
		boosts: {
			spd: -2,
		},
		secondary: null,
	...
	oblivionwing: {
		...
		pp: 10,
		priority: 0,
		flags: {protect: 1, mirror: 1, distance: 1, heal: 1, metronome: 1, drain: 1},
		drain: [3, 4],
		secondary: null,
		target: "any",
		type: "Flying",
	...
	overdrive: {
		...
		pp: 10,
		priority: 0,
		flags: {protect: 1, mirror: 1, vibration: 1, bypasssub: 1},
		secondary: null,
		target: "allAdjacentFoes",
		type: "Electric",
	},
	...
	perishsong: {
		...
		pp: 5,
		priority: 0,
		flags: {vibration: 1, distance: 1, bypasssub: 1, metronome: 1},
	...
	roar: {
		...
		pp: 20,
		priority: -6,
		flags: {reflectable: 1, mirror: 1, vibration: 1, bypasssub: 1, allyanim: 1, metronome: 1, noassist: 1, failcopycat: 1},
		forceSwitch: true,
		secondary: null,
		target: "normal",
		type: "Normal",
	...
	rollout: {
		...
		pp: 20,
		priority: 0,
		flags: {contact: 1, protect: 1, mirror: 1, metronome: 1, failinstruct: 1, noparentalbond: 1, spin: 1},
		onModifyMove(move, pokemon, target) {
	...
	spikes: {
		...
		condition: {
			...
			onEntryHazard(pokemon) {
				if (!pokemon.isGrounded() || pokemon.hasItem('heavydutyboots') || pokemon.hasItem('battlearmor')) return;
	...
	teeterdance: {
		...
		pp: 20,
		priority: 0,
		flags: {protect: 1, mirror: 1, dance: 1, metronome: 1, spin: 1},
		volatileStatus: 'confusion',
		secondary: null,
		target: "allAdjacent",
		type: "Normal",
	...
	throatchop: {
		...
		condition: {
			...
			onDisableMove(pokemon) {
				for (const moveSlot of pokemon.moveSlots) {
					if (this.dex.moves.get(moveSlot.id).flags['vibration']) {
						pokemon.disableMove(moveSlot.id);
					}
				}
			},
			onBeforeMovePriority: 6,
			onBeforeMove(pokemon, target, move) {
				if (!move.isZ && !move.isMax && move.flags['vibration']) {
	...
	toxicspikes: {
		...
		condition: {
			...
			onEntryHazard(pokemon) {
				...
				} else if (pokemon.hasType('Steel') || pokemon.hasItem('heavydutyboots')|| pokemon.hasAbility('battlearmor')) {
					return;
	...
	twister: {
		...
		pp: 20,
		priority: 0,
		flags: {protect: 1, mirror: 1, metronome: 1, wind: 1, spin: 1},
		secondary: {
			chance: 20,
			volatileStatus: 'flinch',
		},
	...
	uproar: {
		...
		pp: 10,
		priority: 0,
		flags: {protect: 1, mirror: 1, vibration: 1, bypasssub: 1, metronome: 1, nosleeptalk: 1, failinstruct: 1},
		self: {
